...
const PresetsInfoFile = "./data/presets.json"
const ScheduleInfoFile = "./data/schedule.json"
...
type PresetMessageInfo struct {
	NoWarningsMessage   string `json:"nowarningsmessage"`
	MidWarningsMessage  string `json:"midwarningsmessage"`
	ManyWarningsMessage string `json:"manywarningsmessage"`
	MaxWarnings			int    `json:"maxwarnings"`
	MaxActivations		int    `json:"maxactivations"`
}

type PresetMessageCommand struct {
	Command             string `json:"command"`
	Info				PresetMessageInfo	`json:"info"`
}

var PresetMessages PresetMessageInfo

func SavePresetsToFile() {
	marshaledData, _ := json.Marshal(PresetMessages)
	_ = ioutil.WriteFile(PresetsInfoFile, marshaledData, 0644)
	fmt.Printf("Schedule info file written")
}

func LoadPresetsFromFile() {
	file, err := ioutil.ReadFile(PresetsInfoFile)
	info := &PresetMessageInfo{
		"¡Lo habéis hecho muy bien!, seguid así",
		"No está mal, pero hay que seguir mejorando",
		"Muy mal, lo volveremos a intentar mañana...",
		3,
		1,
	}
	if err == nil {
		...
		fmt.Println("No presets info file found")
	}
	PresetMessages = *info
}
...
type ScheduleRequestCommand struct {
	Command string `json:"command"`
}

type ScheduleInfo struct {
	NumHours int                `json:"numhours"`
	Slots    []ScheduleSlotInfo `json:"slots"`
}

var SchoolSchedule ScheduleInfo

func (schedule *ScheduleInfo) SetSlot(hourNumber int, startHour int, startMinute int, endHour int, endMinute int) {
	if hourNumber >= len(schedule.Slots) {
		for len(schedule.Slots) <= hourNumber {
			schedule.Slots = append(schedule.Slots, ScheduleSlotInfo{})
		}
	}
	schedule.Slots[hourNumber].Start.Hour = startHour
	schedule.Slots[hourNumber].Start.Minute = endHour
	schedule.Slots[hourNumber].End.Hour = endHour
	schedule.Slots[hourNumber].End.Minute = endMinute
}

func SaveScheduleToFile() {
	marshaledData, _ := json.Marshal(SchoolSchedule)
	_ = ioutil.WriteFile(ScheduleInfoFile, marshaledData, 0644)
	fmt.Printf("Schedule info file written")
}

func LoadScheduleFromFile() {
	file, err := ioutil.ReadFile(ScheduleInfoFile)
	info := &ScheduleInfo{}
	if err == nil {
		_ = json.Unmarshal(file, info)
		SchoolSchedule = *info
		fmt.Println("Schedule read from file: " , SchoolSchedule)
	} else {
		fmt.Println("No schedule info file found")
	}
	SchoolSchedule = *info
}
...
func wsProcessBrowserCommand(socket *websocket.Conn, cmd string) {
	...
	var schedule ScheduleCommand
	var preset PresetMessageCommand

	fmt.Println("    >>> received command from browser: " + cmd)

	if strings.HasPrefix(cmd, "{\"command\":\"rename\"") {
	...
	if strings.HasPrefix(cmd, "{\"command\":\"message\"") {
	...
	if strings.HasPrefix(cmd, "{\"command\":\"schedule\"") {
		err := json.Unmarshal([]byte(cmd), &schedule)
		if err == nil {
			fmt.Println("schedule received from web")
			SchoolSchedule = schedule.Info
			SaveScheduleToFile()
		} else {
			fmt.Println("Dis error: ", err)
		}
	}

	if strings.HasPrefix(cmd, "{\"command\":\"requestschedule\"") {
		fmt.Println("Request schedule called")
		bytes, err := json.Marshal(&SchoolSchedule)
		if err == nil {
			socket.WriteMessage(1, bytes)
		} else {
			fmt.Println("request Dis error: ", err)
		}
	}

	if strings.HasPrefix(cmd, "{\"command\":\"presetmessages\"") {
		fmt.Println("presetmessage")
		err := json.Unmarshal([]byte(cmd), &preset)
		if err != nil {
			fmt.Println("presetmessages dis the error: ", err)
		} else {
			PresetMessages = preset.Info
			fmt.Println("  >> Presets set to: " , PresetMessages)
			SavePresetsToFile()
		}
	}

	if strings.HasPrefix(cmd, "{\"command\":\"requestpresets\"") {
		fmt.Println("Request presets called")
		bytes, err := json.Marshal(&PresetMessages)
		if err == nil {
			socket.WriteMessage(1, bytes)
		} else {
			fmt.Println("request presets Dis error: ", err)
		}
	}
